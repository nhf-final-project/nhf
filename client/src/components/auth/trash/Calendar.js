...
export default class Calendar extends Component {
    ...
    renderMealDay = (day, meal) => {
            ...
            if (this.state.calendarPrograms[day] !== undefined) {
                ...
                return ( <
                    div > {
                        recipesDay.map(recipe => < article > < img src = { recipe.recipe.image }
                            /><p>{recipe.recipe.label}</p > < /article>)} <
                            /div>
                        )
                    }
                    else {
                        ...
                        /div> {
...

                            {
                                (recipes.length !== 0) && recipes.map((recipe, idx) => {
                                    ...
                                    return <article className = "user-recipe"
                                    key = { idx } > < img src = { recipe.image }
                                    alt = { recipe.label }
                                    /><h4>{recipe.label}</h
                                    4 > < p > Calories: { calorieDecimals } < /p><p>Quantity:{recipe.totalWeight} g</p > < button type = "submit"
                                    className = "btn btn-primary"
                                    onClick = {
                                        (e) => this.handleSubmit(e, recipe._id) } > Add Recipe < /button></article >
                                })
                            } <
                            /div>
